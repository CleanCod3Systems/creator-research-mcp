...
This document describes the system as it is actually built and running today. An earlier
design draft explored a much larger Python-based pipeline (local Whisper transcription, OCR,
scene detection, an async job queue, a self-hosted AI engine). That design was **never
implemented** — it was superseded early on by a simpler, cheaper approach, described below.
If you find references to that older plan elsewhere, treat this document as the source of
truth for what actually exists in the codebase.
...
- The server has zero server-side AI dependencies (no Ollama, no OpenAI/Anthropic SDK calls).
...
- **`ports/provider.ts`** — the `ContentProvider` interface every adapter implements:
  `matches(url)`, `classify(url)`, `capabilities()`, `fetchMetadata`, `fetchText`, `fetchMedia`,
  and the optional `listItems`/`fetchComments`. `ProviderCapabilities.reliability` is one of
...
`yt-dlp`/`ffmpeg` invocations always use `execFile` with argument arrays — never a shell string —
so a malicious URL can't inject shell commands.
...
Three tables — `jobs`, `cache_entries`, `worker_heartbeats` — are wired up in
`apps/mcp-server/src/context.ts` but not read or written by any currently registered tool. They
are leftovers from an earlier local worker/queue pipeline that was removed; nothing in this repo
...
- No Whisper, no OCR, no scene detection, no video/audio processing pipeline. If a platform has
  no reliable free way to get structured data (Instagram profile listing, Twitter/X threads,
  LinkedIn behind the login wall), the answer is an honest limitation surfaced through the
  `capabilities` tool — not a heavier scraping/processing pipeline.
